...
void DibujarVentanas(struct ventana[][5], unsigned short int, SDL_Surface *, SDL_Surface*);
...
int main(int argc, char *argv[]) {
	...
	const char 
		...
		felixd2_bmp[] = "Sprites/felix_d2.bmp",
		felixi2_bmp[] = "Sprites/felix_i2.bmp",
		ralph1_bmp[] = "Sprites/rahlp_1.bmp",
		ralph2_bmp[] = "Sprites/rahlp_2.bmp",
		ralph3_bmp[] = "Sprites/rahlp_3.bmp",
		ralph4_bmp[] = "Sprites/rahlp_4.bmp",
		ralph5_bmp[] = "Sprites/rahlp_5.bmp",
		ralph6_bmp[] = "Sprites/rahlp_6.bmp",
		pajaro1_bmp[] = "Sprites/pajaro_1.bmp",
		pajaro2_bmp[] = "Sprites/pajaro_2.bmp",
		roca1_bmp[] = "Sprites/roca1.bmp",
	...
	for (fila = 3; fila > 0; fila--) {
		for (columna = 0; columna < 5; columna++) {
			CargarVentanasDelTramo(&ventanas_tramo1[fila-1][columna],
					ventana_x, ventana_y, columna + (fila-1) * 5, 'N', fila-1, columna);
			ventana_x += 80;
		}
...
//Dibujo la paloma
		if(pajaro == pajaro_1)
			...
			pajaro = pajaro_1;
		Dibujar(ventanas_tramo1[pajaro_posicion.fila][pajaro_posicion.columna].x, ventanas_tramo1[pajaro_posicion.fila][pajaro_posicion.columna].y, pajaro, superficie );
...
//Dibujo a Ralph
		...
		if (felix_posicion.fila == 99)
			...
			Dibujar(ventanas_tramo1[felix_posicion.fila][felix_posicion.columna].x, ventanas_tramo1[felix_posicion.fila][felix_posicion.columna].y, felix1, superficie);
		}
		if(felix2 == NULL)
			felix2 = felix_d2;			
...
void CargarVentanasDelTramo(struct ventana *ventana, unsigned short int x,
		unsigned short int y, unsigned short int nro, char N_S, unsigned short int f, unsigned short int c) {
	ventana->x = x;
	ventana->y = y;
	ventana->fila = f;
	ventana->columna = c;
...
void DibujarVentanas(struct ventana ventanas[][5], unsigned short int cant_filas, SDL_Surface *ventana_sana, SDL_Surface *superficie){
...
unsigned short int columna = 0;
	...
	for (fila = cant_filas; fila > 0; fila--) {
		for (columna = 0; columna < 5; columna++) {
			...
			Dibujar(ventanas[fila-1][columna].x, ventanas[fila-1][columna].y, ventana_sana, superficie);
		}
	}
}
