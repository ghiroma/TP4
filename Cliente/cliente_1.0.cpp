...
void CargarVentanasDelTramo(struct ventana*, unsigned short int,
		unsigned short int, unsigned short int, char);
void *EscuchaTeclas(void *);
void *EscuchaServidor(void *);
void *DibujarPantalla(void *); //Se usará mas adelante.

SDL_Surface *superficie, *pared, *ventana, *puerta, *felix_d, *felix_i;
...
int main(int argc, char *argv[]) {
	...
	const char pared_bmp[] = "Sprites/pared_grande_tramo1.bmp", ventana_bmp[] =
			"Sprites/ventana_grande.bmp", puerta_bmp[] =
			"Sprites/puerta_grande.bmp", felixd_bmp[] = "Sprites/felix_d.bmp",
...
// Las empiezo a numerar desde arriba.
		...
		for (fila = 0; fila < 2; fila++) {
			for (columna = 0; columna < 5; columna++) {
				...
				CargarVentanasDelTramo(&ventanas_tramo1[fila][columna],
						ventana_x, ventana_y, columna + fila * 5, 'N');
				ventana_x += 80;
			}
...
//1
		CargarVentanasDelTramo(&ventanas_tramo1[2][0], ventana_x, ventana_y, 10,
				'N');
		Dibujar(ventana_x, ventana_y, ventana, superficie);
		ventana_x += 80;
//
...
//2
		CargarVentanasDelTramo(&ventanas_tramo1[2][1], ventana_x, ventana_y, 11,
				'N');
...
//3
		CargarVentanasDelTramo(&ventanas_tramo1[2][3], ventana_x, ventana_y, 13,
				'N');
		Dibujar(ventana_x, ventana_y, ventana, superficie);
		ventana_x += 80;
//
...
//4
		CargarVentanasDelTramo(&ventanas_tramo1[2][4], ventana_x, ventana_y, 14,
				'N');
		Dibujar(ventana_x, ventana_y, ventana, superficie);
//
			...
			for (i = 0; i < 3; i++)
				for (j = 0; j < 5; j++)
					if (ventanas_tramo1[i][j].numero == felix_ventana) {
						Dibujar(ventanas_tramo1[i][j].x,
								ventanas_tramo1[i][j].y, felix_d, superficie);
					}
		}
		SDL_Flip(superficie);
		usleep(250000);
...
void* EscuchaServidor(void *arg) {
	...
	while (salir == 'N') {
		...
		if (felix_ventana == 99) {
			...
			if ((felix_ventana + 1) != 5 && (felix_ventana + 1) != 10
					&& (felix_ventana + 1) != 15) {
				felix_ventana++;
			}
		}
