...
Semaforo sem_inicializarTemporizador ((char*) "/sem_inicializarTemporizador",
				      0);
...
SDL_Surface *screen, *background, *tiempo, *jugadores, *infoJugador_part1,
    *infoJugador_part2;
TTF_Font *font;
 ...
 */
void
getConfiguration (unsigned int* port, string* ip, int* duracionTorneo,
		  int* tiempoInmunidad, int* cantVidas)
{
  string content;
  string line;
 ...
 */
void
agregarJugador (Jugador* nuevoJugador)
{
  ...
  for (map<int, Jugador*>::iterator it = listJugadores.begin ();
      it != listJugadores.end (); it++)
    {
  ...
  for (map<int, Jugador*>::iterator it2 = listJugadores.begin ();
      it2 != listJugadores.end (); it2++)
    {
  ...
  while (!allMatchsFinished)
    {
      ...
      for (list<datosPartida>::iterator it = partidasActivas.begin ();
	  it != partidasActivas.end (); it++)
	{

	  auxSemSHMPartida.setSem_t (it->semaforo_pointerSem_t_Partida);
	  cout << "TIME WAIT IN: cant partACvtivas: " << partidasActivas.size ()
	      << endl;
	  cout << "SHM Lecturas Fallidas: " << it->lecturasFallidasSHM_Partida
	      << endl;
	  //cout << "SHM KeelAlivePartida: " << resumenPartida->keepAlivePartida
	    //  << endl;
	  //if (auxSemSHMPartida.timedWait(400000) == 0) {
	  if (sem_trywait (auxSemSHMPartida.getSem_t ()) == 0)
	    {
	      //Pudo acceder a la SHM
	      cout << "Accedio por el Sem SHM" << endl;

	      resumenPartida = (struct puntajesPartida *) shmat (it->idShm,
								 (char *) 0, 0);
		...
		{
		  cout << "+5 Fallas -> entra por el Semaforo que lo habilito"
		      << endl;
		  ...
		  if (resumenPartida->jugando == false)
			...
			{
			  listJugadores[resumenPartida->idJugador1]->Puntaje +=
			      resumenPartida->puntajeJugador1;
			  ...
			  if (listJugadores.count (resumenPartida->idJugador2)
			      == 0)
			    {
			...
			{
			  listJugadores[resumenPartida->idJugador2]->Puntaje +=
			      resumenPartida->puntajeJugador2;
			  listJugadores[resumenPartida->idJugador2]->CantPartidasJugadas++;

			  //si el otro jugador se desconecto asumo que este gano porque el otro avandono
			  if (listJugadores.count (resumenPartida->idJugador1)
			      == 0)
			    {
		      ...
		      if (listJugadores.count (resumenPartida->idJugador1) == 1
			  && listJugadores.count (resumenPartida->idJugador2)
			      == 1)
			{
			  if ((listJugadores[resumenPartida->idJugador1]->Puntaje
			      / listJugadores[resumenPartida->idJugador1]->CantPartidasJugadas)
			      > (listJugadores[resumenPartida->idJugador2]->Puntaje
				  / listJugadores[resumenPartida->idJugador2]->CantPartidasJugadas))
			    {
		  ...
		  else
		    {
		      ...
		      listJugadores[resumenPartida->idJugador1]->Jugando =
			  false;
		      listJugadores[resumenPartida->idJugador2]->Jugando =
			  false;
		    }
	      ...
	      if (it->lecturasFallidasSHM_Partida >= 5)
		{
	      ...
	      cout << "NO PUDO ACCEDER AL SEMAFORO WAIT PORQUE ESTA BLOQUEADO"
		  << endl;
	      it->lecturasFallidasSHM_Partida++;
	    }
	}
      ...
      cout << "keepalive torneos y partidas mutex  todasLasPartidasFinalizadas"
	  << endl;
      allMatchsFinished = todasLasPartidasFinalizadas;
      pthread_mutex_unlock (&mutex_todasLasPartidasFinalizadas);
      cout << "keepalive torneos y partidas unmutex todasLasPartidasFinalizadas"
	  << endl;
...
modoGrafico (void* data)
{
  ...
  screen = SDL_SetVideoMode (ANCHO_PANTALLA_SERVIDOR, ALTO_PANTALLA_SERVIDOR,
			     BPP_SERVIDOR, SDL_HWSURFACE);
  if (screen == NULL)
    {
  ...
  while (true)
	  ...
	  it != listJugadores.end (); it++)
	{
	  if (it->second->CantPartidasJugadas > 0)
	    {
	      it->second->Promedio = it->second->Puntaje
		  / it->second->CantPartidasJugadas;
	      //ordena en un map los promedios con los ID
	      rankings.insert (
		  pair<float, int> (it->second->Promedio, it->second->Id));
	    }
	  else
	    {
      ...
      posDestino.y = 220;
      int i = 0;
      for (multimap<float, int>::iterator it = rankings.begin ();
	  it != rankings.end (); it++)
	{
	  sprintf (txtInfoJugador, "%02d           %-10.10s", (i + 1),
		   listJugadores[it->second]->Nombre.c_str ());
	  infoJugador_part1 = TTF_RenderText_Solid (font, txtInfoJugador,
						    colorNegro);
	  posDestino.x = 45;
	  SDL_BlitSurface (infoJugador_part1, NULL, background, &posDestino);
	  sprintf (txtInfoJugador, "%06d        %02d      %02d",
		   listJugadores[it->second]->Puntaje,
		   listJugadores[it->second]->PartidasGanadas,
		   listJugadores[it->second]->PartidasPerdidas);
	  infoJugador_part2 = TTF_RenderText_Solid (font, txtInfoJugador,
						    colorNegro);
	  posDestino.x = 400;
...
keepAliveJugadores (void*)
{
  ...
  while (!partidasTerminadas)
	  ...
	  it != listJugadores.end (); it++)
	{
	  ...
	  it->second->SocketAsociado->SendNoBloq (message.c_str (),
						  message.length ());
	  readDataCode = it->second->SocketAsociado->ReceiveBloq (
	      buffer, (LONGITUD_CODIGO + LONGITUD_CONTENIDO));

	  cout << "0000000000000-----  readDataCode: " << readDataCode << endl;
	  if (readDataCode == 0)
	    {
	      //el jugador se desconecto
	      cout << "0000000000000-----  Voy a eliminar a: " << it->first
		  << endl;
  ...
  while (true)
    {
      ...
      cout << "_____________________" << endl;
      cSocket->ReceiveBloq (nombreJugador,
			    (LONGITUD_CODIGO + LONGITUD_CONTENIDO));
      clientId++;
      agregarJugador (new Jugador (clientId, nombreJugador, cSocket));
      cout << "Se agrega el jugador NRO:" << clientId << " NOMBRE: "
	  << nombreJugador << endl;
      cout
	  ...
	  it != listJugadores.end (); it++)
	{
	  ...
	  if (idOponente > 0)
	      ...
	      if (nroPartida == 1)
		{
	      ...
	      if (key == -1)
		{
		  cout << "Error al generar clave de memoria compartida"
		      << endl;
		  break;
		}
	      ...
	      if (idShm == -1)
		{
	      ...
	      cout << "SERV TORNEO Partida: " << puertoPartida << "  ID SHM: "
		  << idShm << endl;

	      //inicializo el BLOQUE DE SHM
	      puntajesPartida* resumenPartida =
		  (struct puntajesPartida *) shmat (idShm, (char *) 0, 0);
	      ...
	      string nombreSemaforoPartida = "/" + intToString (puertoPartida)
		  + "_Partida";
	      string nombreSemaforoTorneo = "/" + intToString (puertoPartida)
		  + "_Torneo";

	      //cout<<"ATENCION!!! Seteando semaforos"<<endl;

	      Semaforo* semaforoPartida = new Semaforo (
		  nombreSemaforoPartida.c_str (), 1);
	      Semaforo* semaforoTorneo = new Semaforo (
		  nombreSemaforoTorneo.c_str (), 0);
	      structuraDatosPartida.semaforo_pointerSem_t_Partida =
		  semaforoPartida->getSem_t ();
	      structuraDatosPartida.semaforo_pointerSem_t_Torneo =
		  semaforoTorneo->getSem_t ();

	      cout << "SERV TORNEO nombre SEM Torneo"
		  << semaforoTorneo->getName () << endl;
	      cout << "SERV TORNEO nombre SEM Partida"
		  << semaforoPartida->getName () << endl;
	      cout << "SERV TORNEO Sem_t * semaforo Torneo: "
		  << semaforoTorneo->getSem_t () << endl;
	      cout << "SERV TORNEO Sem_t * semaforo Partida: "
		  << semaforoPartida->getSem_t () << endl;
	      ...
	      listJugadores[idJugador]->SocketAsociado->SendNoBloq (
		  message.c_str (), message.length ());
	      listJugadores[idOponente]->SocketAsociado->SendNoBloq (
		  message.c_str (), message.length ());

	      if ((pid = fork ()) == 0)
		{
		  //Proceso hijo. Hacer exec
		  cout << "J" << idJugador << " Crear Partida en el puerto: "
		      << (puertoPartida - 1) << " (" << idJugador << "vs"
		      << idOponente << ")" << endl;
		  ...
		  char *argumentos[] =
		    { auxPuertoNuevaPartida, auxCantVidas, NULL };
		  execv ("../Servidor Partida/Debug/Servidor Partida",
			 argumentos);
...
void
eliminarMemoriaCompartida (void * bloqueCompartido, int IdBloqueCompartido)
{
