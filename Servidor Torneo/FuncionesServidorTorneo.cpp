...
SIG_CHLD (int inum)
{
  ...
  if (resumenPartida->partidaFinalizadaOK == true)
      ...
      if (listJugadores.count (resumenPartida->idJugador2) == 1)
	{
      ...
      if (listJugadores.count (resumenPartida->idJugador1) == 1
	  && listJugadores.count (resumenPartida->idJugador2) == 1)
	{
	  ...
	  it != listJugadores.end (); it++)
	{
	  ...
	  if (idOponente > 0)
	      ...
	      if (idShm == -1)
		{
	      ...
	      if ((pid = fork ()) == 0)
		{
		  cout << "partida nro: " << nroPartida << " FORK- PID:"
		      << getpid () << endl;
...
liberarRecursos ()
{
  ...
  for (list<datosPartida>::iterator it = partidasActivas.begin ();
      it != partidasActivas.end (); it++)
    {
      struct puntajesPartida* resumenPartida =
	  (struct puntajesPartida *) shmat (it->idShm, (char *) 0, 0);
